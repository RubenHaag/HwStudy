...
public class GameOfLive extends JavaKaraProgram {
  public short runden = 1000;
  public int feldgröße = 50;
  public int fortpflanzungPflanz = 3;
  public int fortpflanzungFress = 2;
  public int jagdwahrscheinlichkeit = 4;
        ...
        if(world.isMushroom(zeile, spalte)){
          for(int x = zeile-1; x<=(zeile+1); x++){
            for(int y = spalte-1; y<=(spalte+1); y++){
              if(!gefressen){
                if (world.isLeaf(x , y)){
                  zufall=tools.random(jagdwahrscheinlichkeit);
                  if(zufall==0){
  ...
  // hier kommt das Hauptprogramm hin, z
    world.setSize(feldgröße,feldgröße);
    zufallsFeld();
